...
use opencascade::{
    adhoc::AdHocShape,
    angle::Angle,
    primitives::{self,Shape, Direction, Face, Solid,Edge},
    workplane::Workplane,
};
...
pub fn main() {
    ...
    let mut ring = rim(outer,thickness,height);
    //let sp = spoke(outer-thickness/2.0,thickness/2.0,20.0);
    ...
    let (mut fly , mut hub_con) = fly.union_shape(&the_hub);
...
fn rim(outer:f64 , thickness: f64,height: f64) -> Shape { 
    let mut outer_wire = Workplane::xy().circle(0., 0., outer);
...
fn spoke(length: f64,thickness:f64,rotation:f64) -> Shape { 
    ...
    let mut inner_wire = wp.circle(0., 0., thickness/2.0);

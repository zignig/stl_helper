...
use opencascade::{
    adhoc::AdHocShape,
    primitives::{self, Direction, Face, Solid},
    workplane::Workplane,
};
...
pub fn main() {
    ...
    let mut inner = bowly_shape(bot_rad - thickness, top_rad - thickness, height, thickness);
...
fn bowly_shape(bot_rad: f64, top_rad: f64, height: f64, offset: f64) -> primitives::Shape {
    let mut bottom = Workplane::xy().circle(0., 0., bot_rad);
